...
export const CategoriesManagement: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.nombre.trim()) {
      showToast('El nombre es obligatorio', { variant: 'warning' });
      return;
    }

    if (formData.nombre.trim().length < 2) {
      showToast('El nombre debe tener al menos 2 caracteres', { variant: 'warning' });
      return;
    }
    ...
    try {
      if (editingId) {
        ...
        showToast('Categoría actualizada correctamente', { variant: 'success' });
      } else {
        // Crear
        ...
        showToast('Categoría creada correctamente', { variant: 'success' });
      }
  ...
  const handleDelete = async (id: number, nombre: string) => {
    ...
    try {
      ...
      showToast('Categoría eliminada correctamente', { variant: 'success' });
      loadCategorias();
    } catch (err: unknown) {
